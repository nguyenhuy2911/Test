...
namespace PingYourPackage.API.Controllers
{
    ...
    public class AffiliatesController : ApiController
    {
        ...
        public PaginatedDto<AffiliateDto> GetAffiliates(PaginatedRequestCommand cmd)
        {
            ...
            var affiliates = _shipmentService.GetAffiliates(cmd.Page, cmd.Take);

            return affiliates.ToPaginatedDto(affiliates.Select(af => af.ToAffiliateDto()));
        }
        ...
        public HttpResponseMessage PostAffiliate(AffiliateRequestModel requestModel)
        {
            ...
            var createdAffiliateResult = _shipmentService.AddAffiliate(requestModel.UserKey.Value, requestModel.ToAffiliate());
            ...
            var response = Request.CreateResponse(HttpStatusCode.Created, createdAffiliateResult.Entity.ToAffiliateDto());

            response.Headers.Location = new Uri(Url.Link("DefaultHttpRoute", new { key = createdAffiliateResult.Entity.Key }));
