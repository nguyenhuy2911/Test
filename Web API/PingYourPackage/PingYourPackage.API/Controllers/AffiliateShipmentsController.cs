...
namespace PingYourPackage.API.Controllers
{
    ...
    public class AffiliateShipmentsController : ApiController
    {
        ...
        public PaginatedDto<ShipmentDto> GetShipments(Guid key, PaginatedRequestCommand cmd)
        {
            ...
            var shipments = _shipmentService.GetShipments(cmd.Page, cmd.Take, affiliateKey: key);

            return shipments.ToPaginatedDto(shipments.Select(sh => sh.ToShipmentDto()));
        }

        [EnsureShipmentOwnership]
        public ShipmentDto GetShipment(Guid key, Guid shipmentKey, [BindShipment]Shipment shipment)
        {
        ...
        public HttpResponseMessage PostShipment(Guid key, ShipmentByAffiliateRequestModel requestModel)
        {
            ...
            var createdShipmentResult = _shipmentService.AddShipment(requestModel.ToShipment(key));
        ...
        [EnsureShipmentOwnership]
        [EmptyParameterFilter("requestModel")]
        public ShipmentDto PutShipment(Guid key, Guid shipmentKey, ShipmentByAffiliateUpdateRequestModel requestModel, [BindShipment]Shipment shipment)
        {
        ...
        [EnsureShipmentOwnership]
        public HttpResponseMessage DeleteShipment(Guid key, Guid shipmentKey, [BindShipment]Shipment shipment)
        {
